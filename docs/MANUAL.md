...
Auto-generated by cobra. Emits a shell completion script to stdout. Since v1.5.2 the script knows about installed models, running services, recipe names, and config keys — so tab cycles through real values instead of falling through to filename completion.

```bash
# zsh — once per machine
llamactl completion zsh > "${fpath[1]}/_llamactl"
# (or any directory on your $fpath; ~/.zsh-cache/ is common)
# Then in a new shell:
llamactl serve <TAB>      # → installed model ids
llamactl stop <TAB>       # → currently-running run names
llamactl serve qwen3-1.7b --recipe <TAB>   # → chat code long-context low-memory agent
llamactl config get <TAB>                  # → 6 config keys
llamactl add <TAB>        # → preferred-id short names (skips suggestions if you type a /)
```
...
Run `llamactl completion --help` for additional shell-specific notes from cobra.

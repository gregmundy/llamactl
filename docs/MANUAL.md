...
llamactl remove <TAB>                      # → installed model ids
llamactl serve qwen3-1.7b --recipe <TAB>   # → chat code long-context low-memory agent
llamactl serve qwen3-1.7b --draft <TAB>    # → installed ids (excluding the main)
llamactl config get <TAB>                  # → 6 config keys
llamactl add <TAB>                         # → preferred-id short names (skips once you type /)
llamactl fit --speculative <TAB>           # → installed model ids
```
